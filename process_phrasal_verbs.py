...
class Extractor:
    def extract_phrasal_verbs(self, rows):
        ...
        for row in rows:
            ...
            if self.is_verb(current_tag) and len(rows) > current_index and row[0] == rows[current_index + 1][0]:
...
cursor = conn.cursor()
cursor.execute("SELECT * FROM trabalho_ia.sentences ORDER BY source_file ASC, id ASC, part ASC")
